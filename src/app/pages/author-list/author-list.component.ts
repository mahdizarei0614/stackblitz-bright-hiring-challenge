...
export class AuthorListComponent implements OnInit {
  private _authorFacade = inject(DataFacadeService);
  private _fb = inject(FormBuilder);

  authors$ = this._authorFacade.authors$;
  totalAuthors$ = this._authorFacade.totalAuthors$;
  searchText = signal('');
  ...
  loadAuthors(event: { rows: number; first?: number }) {
    console.info(event);
    this._authorFacade.loadAuthors(this.searchText(), this.selectedSortType());
  }
  ...
  saveAuthor() {
    if (this.authorForm.valid) {
      ...
      this._authorFacade.loadAuthors();
